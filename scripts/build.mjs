...
const renderInlineMarkdown = (value = '') => esc(value).replace(/`([^`]+)`/g, '<code>$1</code>');
const isTableDivider = (line) => /^\|?\s*:?-{3,}:?\s*(?:\|\s*:?-{3,}:?\s*)+\|?\s*$/.test(line);
const splitTableRow = (line) => line.trim().replace(/^\|/, '').replace(/\|$/, '').split(' | ').map((cell) => cell.trim());
const renderMarkdownTables = (value = '') => {
  ...
  for (let index = 0; index < lines.length;) {
    if (lines[index]?.trim().startsWith('|') && isTableDivider(lines[index + 1] ?? '')) {
      ...
      chunks.push(`<table><thead><tr>${header.map((cell) => `<th>${renderInlineMarkdown(cell)}</th>`).join('')}</tr></thead><tbody>${rows.map((row) => `<tr>${row.map((cell) => `<td>${renderInlineMarkdown(cell)}</td>`).join('')}</tr>`).join('')}</tbody></table>`);
      continue;
    }
    const paragraph = [];
    while (index < lines.length && !(lines[index]?.trim().startsWith('|') && isTableDivider(lines[index + 1] ?? ''))) {
      paragraph.push(lines[index]);
      index += 1;
    }
    const text = paragraph.join('\n').trim();
    if (text) chunks.push(`<p>${renderInlineMarkdown(text).replaceAll('\n', '<br>')}</p>`);
  }
  return chunks.join('');
};
const copyScript = `<script>
  document.querySelectorAll('.copy-expression').forEach((button) => {
    button.addEventListener('click', async () => {
      const code = button.closest('.expression-section')?.querySelector('code')?.textContent ?? '';
      if (!code) return;
...
await Promise.all(data.recipes.map((recipe) => writeFile(resolve(recipesDir, `${slug(recipe)}.html`), layout(recipe.title, `<article class="recipe-detail"><a class="back-link" href="../chapter.html">← Back to chapters</a><p class="eyebrow">Recipe #${recipe.number}</p><h1>${esc(recipe.title)}</h1><p class="hero-description">${esc(recipe.description || recipe.summary)}</p><dl class="recipe-meta"><div><dt>적용 속성</dt><dd>${esc(recipe.property)}</dd></div><div><dt>결과</dt><dd>${esc(recipe.result)}</dd></div></dl><section class="expression-section"><div class="expression-heading"><h2>Expression</h2><button class="copy-expression" type="button">Copy Expression</button></div><pre><code>${esc(recipe.expression)}</code></pre></section><h2>조절 가능한 값</h2><div class="markdown-block">${renderMarkdownTables(recipe.controls)}</div><h2>활용 예시</h2><p>${esc(recipe.examples)}</p><h2>주의사항</h2><p>${esc(recipe.cautions)}</p><h2>초보 TIP</h2><p>${esc(recipe.tip)}</p><h2>함께 쓰면 좋은 레시피</h2><p>${esc(recipe.pairs)}</p></article>`, '../'))));
